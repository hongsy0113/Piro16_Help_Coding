    ...
    if answered_user:
        ...
        if answered_user != posted_user and posted_user:
            Alert.objects.create(user=posted_user, content="[{}] 질문글에 답변이 달렸어요.".format(
                textwrap.shorten(question.title, width=15, placeholder='...')), alert_type="new_comment_qna", related_id=question.id, time=datetime.now())
    ...
    if replied_user:
        ...
        if replied_user != reply.parent_answer.user and reply.parent_answer.user:
            Alert.objects.create(user=reply.parent_answer.user, content="[{}] 댓글에 대댓글이 달렸어요.".format(
                textwrap.shorten(reply.parent_answer.content, width=15, placeholder='...')), alert_type="new_reply_qna", related_id=question.id, time=datetime.now())
...
def update_group_create(group, created_user):
    if group.mode == 'PUBLIC':
        Alert.objects.create(user=created_user, content="[{}] 그룹이 만들어졌어요. 가입 신청이 오면 대기자 명단 창에서 수락 또는 거절해주세요!".format(
            textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_create", related_id=group.id, time=datetime.now())
    else:
...
def update_private_group_join(group, registered_user):
    Alert.objects.create(user=group.maker, content="[{}] 님이 [{}] 그룹에 가입했어요.".format(
        registered_user.nickname, textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_join", related_id=group.id, time=datetime.now())
    Alert.objects.create(user=registered_user, content="[{}] 그룹에 가입되었어요. 그룹에서 첫 글을 남겨보세요!".format(
        textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_join", related_id=group.id, time=datetime.now())
...
def update_public_group_register(group, registered_user):
    Alert.objects.create(user=group.maker, content="[{}] 님이 [{}] 그룹에 가입하고 싶어해요. 대기자 명단 창에서 수락 또는 거절해주세요!".format(
        registered_user.nickname, textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_register", related_id=group.id, time=datetime.now())
    Alert.objects.create(user=registered_user, content="[{}] 그룹 가입을 요청했어요. 승인될 때까지 기다려주세요!".format(
        textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_register", related_id=group.id, time=datetime.now())
...
def update_public_group_reject(group, registered_user):
    Alert.objects.create(user=registered_user, content="[{}] 그룹 가입 신청이 거절되었어요. 아쉽지만 다른 그룹을 찾아보아요!".format(
        textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_reject", related_id=group.id, time=datetime.now())
...
def update_change_group_maker(group, original_maker, new_maker):
    Alert.objects.create(user=original_maker, content="[{}] 그룹에서 탈퇴했어요.".format(
        textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_drop", related_id=group.id, time=datetime.now())
    if group.mode == 'PUBLIC':
        Alert.objects.create(user=new_maker, content="[{}] 그룹 대표가 되었어요. 가입 신청이 오면 대기자 명단 창에서 수락 또는 거절해주세요!".format(
            textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_maker", related_id=group.id, time=datetime.now())
    else:
...
def update_delete_group(group):
    for member in group.members.all():
        Alert.objects.create(user=member, content="[{}] 그룹이 삭제되었어요.".format(
            textwrap.shorten(group.name, width=15, placeholder='...')), alert_type="group_delete", related_id=group.id, time=datetime.now())
...
def update_group_comment(post, comment, posted_user, commented_user):
    ...
    if commented_user != posted_user and posted_user:
        Alert.objects.create(user=posted_user, content="[{}] 그룹의 [{}] 글에 댓글이 달렸어요.".format(
            textwrap.shorten(post.group.name, width=15, placeholder='...'), textwrap.shorten(post.title, width=15, placeholder='...')), alert_type="new_comment_group", related_id=str(post.group.id)+" "+str(post.id), time=datetime.now())
    ...
    if replied_user:
        ...
        if replied_user != reply.parent_answer.user and reply.parent_answer.user:
            Alert.objects.create(user=reply.parent_answer.user, content="[{}] 그룹의 [{}] 댓글에 대댓글이 달렸어요.".format(
                textwrap.shorten(post.group.name, width=15, placeholder='...'), textwrap.shorten(reply.parent_answer.content, width=15, placeholder='...')), alert_type="new_reply_group", related_id=str(post.group.id)+" "+str(post.id), time=datetime.now())
